...
K_pcr=[1,3,5,10,25,50,75,100]
K_pls=[1,3,5,10,25,50,75,100]
L_ridge=10**np.linspace(-2,2,9)
L_lasso=10**np.linspace(-2,2,9)
...
for i in range(0,n_rolling_windows):
    ...
    for k in K_pls:
        ...
        y_pred['PLS'].at[i,k] = pls_model.predict(x_test.values.reshape(1, -1))
...
for name in list(y_pred.keys()):
    ...
    plt.title(f'MSFE AR(1) vs. MSFE {name}')
    plt.xlabel('Time')
    plt.ylabel('MSFE')
    plt.grid(True)
    plt.legend()
...
fig, ax = plt.subplots()
for i in range(zeros_opt_10.shape[0]):
    print(i)
    for j in range(zeros_opt_10.shape[1]):
        color = 'red' if zeros_opt_10.iloc[i, j] == 1 else 'grey'
        ax.add_patch(plt.Rectangle((j, -i-1), 1, 1, color=color))
ax.set_xlim(0, zeros_opt_10.shape[1])
ax.set_ylim(-zeros_opt_10.shape[0], 0)
ax.set_yticks([])
ticks = [107,227,347,467]
...
#%% rolling variance
plt.figure(figsize=(15, 9))
plt.plot(y_std.index,y_std-y_std.shift(1), linestyle='-', color = "black")
plt.title('Standard deviation monthly change rolling estimates')
plt.xlabel('Time')
plt.ylabel('SD variation')
plt.grid(True)
